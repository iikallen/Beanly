...
export function PublicOrderClient({ slug }: { slug: string }) {
  ...
  return <main className="public-order-page">
    <header className="public-order-hero"><div><p>{location.station ? `${location.station.kind.replaceAll("_", " ")} · ${location.station.label}` : "Pickup"}</p><h1>{location.location_name}</h1><span aria-live="polite">{location.accepting_orders ? "Accepting orders" : location.unavailable_reason?.replaceAll("_", " ") || "Ordering paused"}</span></div><ShoppingBag aria-hidden="true" /><strong aria-label={`${count} items in cart`}>{count}</strong></header>
