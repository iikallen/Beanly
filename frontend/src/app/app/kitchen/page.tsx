...
export default function KitchenPage() {
  ...
  return <section className="kitchen-content">
    ...
    {view === "BOARD" && permissions.canRead && <div className="kitchen-board" aria-live="polite">
      {!station ? <KitchenEmpty title="No active station" body="A manager can create the first kitchen station in Setup." /> : visibleTickets.length === 0 ? <KitchenEmpty title="Board is clear" body={`Waiting for paid orders · sync ${cursor}`} /> : visibleTickets.map((ticket) => <TicketCard key={ticket.id} ticket={ticket} station={station} now={serverNow} canWork={permissions.canWork} canExpo={permissions.canExpo} busyId={busyId} onStart={(workId) => run(workId, () => api.startKitchenWork(workId, actionId(), organization!.id, accessToken!))} onReady={(workId) => run(workId, () => api.readyKitchenWork(workId, actionId(), organization!.id, accessToken!))} onComplete={() => run(ticket.id, () => api.completeKitchenTicket(ticket.id, actionId(), organization!.id, accessToken!))} onRecall={() => run(ticket.id, () => api.recallKitchenTicket(ticket.id, actionId(), organization!.id, accessToken!))} />)}
    </div>}
...
function TicketCard({ ticket, station, now, canWork, canExpo, busyId, onStart, onReady, onComplete, onRecall }: { ticket: KitchenTicket; station: KitchenStation; now: number; canWork: boolean; canExpo: boolean; busyId: string; onStart: (id: string) => void; onReady: (id: string) => void; onComplete: () => void; onRecall: () => void }) {
  ...
  return <article className={`kitchen-ticket is-${aging.level}`}><header><div><span>#{ticket.order_number} · {ticket.order_type.replace("_", " ")}</span><strong>{ticket.status.replace("_", " ")}</strong></div><time><Clock3 />{clock(aging.elapsedSeconds)}</time></header>
    ...
    {ticket.offline_delayed && <span className="kitchen-delay">Offline delayed</span>}{ticket.note && <p className="kitchen-order-note">{ticket.note}</p>}
