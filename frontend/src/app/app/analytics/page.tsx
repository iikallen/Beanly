...
import { Banknote, PackageOpen, Percent, ReceiptText, ShoppingBag } from "lucide-react";
  ...
  return <>
    ...
    {!scope.canRead ? <AnalyticsAccessState /> : query.error ? <AnalyticsError message={query.error} onRetry={query.retry} /> : !data ? <AnalyticsLoading /> : data.overview.paid_orders === 0 && data.inventory.rows.length === 0 ? <AnalyticsEmpty /> : <>
      ...
      <div className="analytics-overview-grid">
        <section className="analytics-panel analytics-top-products"><header><div><span>Sales mix</span><h2>Top Products</h2></div><Link href="/app/analytics/products">View all</Link></header>{data.products.rows.length ? <ol>{data.products.rows.map((row, index) => <li key={row.product_id}><span><b>{index + 1}</b>{row.name}</span><strong>{formatAnalyticsMoney(row.revenue, data.overview.currency_code)}</strong></li>)}</ol> : <p className="analytics-panel-empty">No product sales in this period.</p>}</section>
